...
function showForm(code) {
  if (code != "") {
    ...
    if (cint > 0)
      $("#input-inv").append(
        `<label class="form-label" placeholder="Apellido Materno">Agrega los nombres de tus acompañantes</label>`
      );
    for (let index = 0; index < cint; index++) {
      $("#input-inv").append(
        `<div class='mb-3'><input class='form-control' name='${invInputList[index]}' type='text'/></div>`
      );
    }
    if (cnin > 0) {
      $("#cka").append(
        `<label class="form-label" placeholder="Apellido Paterno">¿Cuántos niños asistirán?</label><input id="cantna" class="form-control" max="${cnin}" name="entry.1096580569" type="text"/>`
      );
      $("#cantna").change(() => {
        if ($("#cantna").val() > cnin) {
          $("#cantna").val(cnin);
